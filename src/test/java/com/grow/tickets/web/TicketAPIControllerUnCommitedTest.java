...
class TicketAPIControllerUnCommitedTest extends TicketAPIControllerTest{
    private static final String ISOLATION_LEVEL = "uncommited";
    ...
    @Test
    public void shouldDirtRead() throws Exception {
        Map<TimePoint, Long> result = tryDirtyRead(ISOLATION_LEVEL);

        assertThat(result.get(TimePoint.START), not(result.get(TimePoint.FINISH)));
    }
    ...
    public void shouldPhantomRead() throws Exception {
        ...
        assertThat(result.get(TimePoint.FINISH), not(result.get(TimePoint.START)));
    }
